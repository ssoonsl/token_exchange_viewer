...
export function SwapInterface() {
  ...
  const { data: tokens = [], isLoading: tokensLoading, error: tokensError } = useGetTokenListQuery();
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg space-y-6">
            ...
            <div className="space-y-3 text-sm">
              ...
              {sourceToken && targetToken && (
                <div className="flex justify-between text-xs text-gray-500 pt-2 border-t">
                  ...
                  <span>
                    1 {sourceToken.symbol} = {calculations.exchangeRate.toFixed(6)} {targetToken.symbol}
                  </span>
                </div>
              )}
            </div>
      ...
      {/* Status Messages */}
      {sourceToken && targetToken && usdAmount && !amountError && !calculations && !isCalculating && (
